            ...
            for (let each in data[item]){ // Where data[item] is the value of each
                if ([
                    ...
                    "sunset"].includes(each)  //Checking if each is in this array of words
                        ...
                        if (each === "temp" || each === "feelslike"){ //Converting farenheight to celsius
                            let current_value = data[item][each];
                            let temp_in_celsius = Math.round((Number(current_value) - 32) * (5/9)); 
                        ...
                        else { // Else print everything else normally
                            let sub_item = document.createElement("li");
...
function change_color(data){
    ...
    let colors = [["#E85D04", "#FFD166"], ["#1D4ED8", "#7DD3FC"], ["#0F4C5C", "#A8DADC"]];
